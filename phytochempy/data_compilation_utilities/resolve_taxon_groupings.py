...
def get_pathway_version_resolved_at_taxon_level(df: pd.DataFrame, pathway: str, taxon_grouping_col: str = 'Genus'):
    """
    :param df: A pandas DataFrame containing the data
    :param pathway: A string specifying the column name of the class
    :param taxon_grouping_col: A string specifying the taxon grouping column name (default is 'Genus')
    ...
    (e.g. genus names) given in the 'taxon_grouping_col' column.
    ...
    num_genera_tested = len(genera_df[taxon_grouping_col].unique().tolist())

    # count labelled species
    counts = genera_df.value_counts(taxon_grouping_col)
    counts.name = 'identified_compounds_count'
    genera_df = pd.merge(genera_df, counts, how='left', left_on=taxon_grouping_col, right_index=True)
    N_class_col = f'identified_{pathway}_count'
    genera_df[N_class_col] = genera_df.groupby([taxon_grouping_col])[pathway].transform('sum')
    mean_col = f'mean_identified_as_{pathway}'
    genera_df[mean_col] = genera_df.groupby([taxon_grouping_col])[pathway].transform('mean')
    ...
    genera_df = genera_df[
        [taxon_grouping_col, 'identified_compounds_count', N_class_col, mean_col, expected_mean_col, factor_col,
         norm_mean_col]]
    genera_df = genera_df.reset_index(drop=True)

    genera_df = genera_df.drop_duplicates(subset=[taxon_grouping_col])

...
class TestGetPathwayVersionResolvedAtTaxonLevel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pathway = 'path'
        self.taxon_grouping_col = 'Genus'

    def test_function_runs_without_error(self):
        get_pathway_version_resolved_at_taxon_level(self.df, self.pathway, self.taxon_grouping_col)

    def test_return_type(self):
        result = get_pathway_version_resolved_at_taxon_level(self.df, self.pathway, self.taxon_grouping_col)
        self.assertIsInstance(result, pd.DataFrame)

    def test_resulting_df_shape(self):
        result = get_pathway_version_resolved_at_taxon_level(self.df, self.pathway, self.taxon_grouping_col)
        self.assertEqual(len(result[self.taxon_grouping_col].unique()), result.shape[0])
    ...
    def test_resulting_df_values(self):
        result = get_pathway_version_resolved_at_taxon_level(self.df, self.pathway, self.taxon_grouping_col)
        for genus, group_df in self.df.groupby(self.taxon_grouping_col):
            row = result[result[self.taxon_grouping_col] == genus].iloc[0]
